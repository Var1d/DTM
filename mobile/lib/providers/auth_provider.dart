...
class AuthProvider with ChangeNotifier {
  ...
  void _setState(AuthState s, {String? err}) {
    _state = s; _errorMessage = err; notifyListeners();
  }
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      ...
      await StorageService.saveTokens(res['data']['access_token'], res['data']['refresh_token']);
      await StorageService.saveUser(res['data']['user']);
      _user = UserModel.fromJson(res['data']['user']);
      _setState(AuthState.authenticated);
      return true;
    } catch (e) {
      _setState(AuthState.error, err: e.toString().replaceAll('Exception: ', ''));
      return false;
    }
  }
